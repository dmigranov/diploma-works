...
int Game::Initialize(HWND window, int width, int height)
{
    ...
    2. Create a render target view of the swap chainТs back buffer,
    ...
    */
    ...
    // we use the swap chainТs GetBuffer method to retrieve a pointer to the swap chainТs 
    // single back buffer. The swap chainТs back buffer is automatically created based on the content of the DXGI_SWAP_CHAIN_DESC variable
    // so we do not need to manually create a texture for this purpose. However we do need to associate the backbuffer to a render target view in order to render to the swap chainТs back buffer.
    ...
    depthStencilStateDesc.DepthEnable = TRUE;       //тест глубины проводитс€
    depthStencilStateDesc.DepthWriteMask = D3D11_DEPTH_WRITE_MASK_ALL;
    depthStencilStateDesc.DepthFunc = D3D11_COMPARISON_LESS;
    depthStencilStateDesc.StencilEnable = FALSE;    //“≈—“ трафарета Ќ≈ ѕ–ќ¬ќƒ»“—я!
