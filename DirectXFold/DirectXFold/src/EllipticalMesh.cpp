...
EllipticalMesh::EllipticalMesh(int nv, VertexPosColor* vertices, int ni, WORD* indices) : Mesh(nv, vertices, ni, indices)
{ }

EllipticalMesh::EllipticalMesh(int nv, VertexPosColor* vertices, int ni, WORD* indices, XMMATRIX world) : Mesh(nv, vertices, ni, indices, world)
{
}


void EllipticalMesh::Render()
{
...
void EllipticalMesh::Render(DirectX::XMMATRIX matrix)
{
    // Input Assembler Stage - unique for every mesh
    const UINT vertexStride = sizeof(VertexPosColor);   //Each stride is the size (in bytes) of the elements that are to be used from that vertex buffer.
    const UINT offset = 0;
...
void EllipticalMesh::Render(std::list<DirectX::XMMATRIX> matrices)
{
    // Input Assembler Stage - unique for every mesh
    const UINT vertexStride = sizeof(VertexPosColor);   //Each stride is the size (in bytes) of the elements that are to be used from that vertex buffer.
    const UINT offset = 0;
