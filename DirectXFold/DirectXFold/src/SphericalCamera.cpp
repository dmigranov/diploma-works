...
const XMMATRIX& SphericalCamera::GetView()
{
	if (m_viewDirty)
	{
		...
		//view - это обратна€ к cameraTransform.
		//чтобы найти камера“рансформ, сначала поворачиваем камеру (слева), потом перемещаем (справа)
		//но (ABC)-1 = C-1 B-1 A-1
		//поэтому при нахождении view вращени€ камеры —ѕ–ј¬ј
		
		//пич йоу:
		//дл€ нахождени€ камера трансформ сначала pitch, потом yaw. то есть там pitch левее
		//но дл€ view питч будет правее
...
void SphericalCamera::ChangePitchYawRoll(double deltaPitch, double deltaYaw, double deltaRoll)
{
	...
	RRoll = XMMatrixRotationQuaternion(XMQuaternionRotationAxis(XMVector4Transform(Vector4(0, 0, 1, 0), R), m_roll));
	R = RRoll * RYaw * RPitch;
